import { AgnoCacheConf, ConfigService } from '@config/env.config';
import { Logger } from '@config/logger.config';
import NodeCache from 'node-cache';
import { createClient, RedisClientType } from 'redis';

export class AgnoCache {
  private readonly logger = new Logger('AgnoCache');
  private readonly conf: AgnoCacheConf;
  private readonly localCache: NodeCache;
  private client: RedisClientType | null = null;

  constructor(configService: ConfigService) {
    this.conf = configService.get<AgnoCacheConf>('AGNO_CACHE') || {
      ENABLED: false,
      URI: '',
      PREFIX_KEY: 'agno-cache',
      TTL: 3600,
    };
